...
public class DetectMinMaxLineCollision_version2 : MonoBehaviour
{
    ...
    private bool lanecentering;

    public bool dontStartEarly = false;
 /*
    ...
    void Start()
    {
       ...
       // lanecentering = handleBarCollider.lanecentering;
    ...
    private void Update()
    {
       ...
       // Debug.Log("physicalSwitch: " + physicalSwitch);
        ...
        if (digitalSwitch == false)// && physicalSwitch == 0)
        {
            StopMotor();
        }

        distanceBetweenObjects = Vector3.Distance(handlebar.transform.position, sphere_path.transform.position);  //ca. 9 wenn ich vor der sphere bin und sogar irwas bei 4 wenn ich dahinter bin; normal 8
        Debug.Log("Distance handle + sphere: " + distanceBetweenObjects);
    ...
    private void SetMotorSpeed(int direction)
    {
        ...
        cmd = 75; //best case not to weak but not too strong
        ...
        if (direction == 1)
        {
            ...
            float interpolatedValue = Mathf.Lerp(40, 60, t);  //40 65
        ...
        else if (direction == -1)
        {
            ...
            // Interpolate the value within the range [min, max] based on the t value
            float interpolatedValueM = Mathf.Lerp(40, 60, tt);  //40 65
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        //if (onTrack == true)
       // {
            if (other.CompareTag("LineMin") && onlyOneTrigger == false)
            {
                //Debug.Log("collider: LineMin");
                minLineCounter++;
            ...
            else if (other.CompareTag("LineMax") && onlyOneTrigger == false)
            {
               // Debug.Log("collider: LineMax");
                maxLineCounter++;
    ...
    {
        if (other.name.StartsWith("Road"))
        {
            onTrack = false;
            Debug.Log("onTrack false");
        }
        else {
            onTrack = true;
            Debug.Log("onTrack true");
        }
        //
    }
